...
$tmarker = date('Y-m-d H:i:s', time());
error_log($tmaker);

error_log("gen mail");
$mailfilial = array(
		//"м. Алексеевская"=>'zakaz@kwikkopy.ru',
	"default"=>'order@pechati.ru',
	...
	"м. Чистые пруды"=>'5176723@mail.ru',
	...
	"М.О., г. Зеленоград"=>'zel@pechati.ru', //6643718@mail.ru
	...
	"Химки"=>'zel@pechati.ru', //6643718@mail.ru
...
function complete_mail() {
	...
	if ($_POST[form]!=7 or $_POST[form]!=8) {
		//$subject = "[ZAKAZ]Заказ на изготовление печатей ($tmarker) - ";
		$subject = "$tmarker - [ZAKAZ] - ";
	} 
	else {
		if ($_POST[form]==7) {
	...
	/*if($_POST['myurl']){
		$subject = $subject." Заказ со страницы(".$_POST['myurl'].")". " - $_POST[adress]";
	}*/
	//$subject = "[ZAKAZ] Заказ печатей".
	$subject = $subject."$_POST[adress]";
