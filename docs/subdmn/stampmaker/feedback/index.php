...
$form['sotrudnichestvo'] = array(
	...
	'cfg' => array(
		'charset' => 'utf-8',
		'subject' => 'Сотрудничество с ПЕЧАТИ.РУ',
		'title' => 'Сотрудничество с ПЕЧАТИ.РУ',
		'ajax' => true,
		'validate' => true,
		'from_email' => 'vadim@pechati.ru',
		'from_name' => '',
		'to_email' => 'vadim@pechati.ru',
		'to_name' => '',
		'geoip' => true,
		'referer' => true,
		'type' => 'html',
...
if(isset($form[$act])) {
   ...
   $sb = array(); // subject и body
    ...
    if(count($error) == 0) {
      ...
      $headers .= "X-Mailer: Feedback, v0.3 (http://www.pechati.ru.ru)\r\n";
	 ...
	 error_log("body completed!!!!!");
	  ...
	  $str = '';
		if (gettype($mail)=='boolean') {
			 $str = ($mail) ? 'true' : 'false';
		}else {
			$str = $mail;
		}
		error_log("check result of send..." . " mail:  " . $str . ' <---');
		//$mail=true;
      if($mail) {
          $jsonBox['ok'] = 1;
          $info[] = $form['cfg']['okay'];
