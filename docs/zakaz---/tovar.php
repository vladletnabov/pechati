<?php require_once "../extdata/variables-filial3.php" ?>
...
dostav = dostav_kur;
	...
	function showfileload(){
					...
					function opendialog(fil,idfil,tov, idtov){
						//var attr = $this.attr("id");
						//
						
						//curentid = $this.attr("id");//.slice($this.attr("id").indexOf("-")+1);
						//setparametrs();
						//$("#dial").dialog("open");
						//var json = JSON.parse(eval(<?php echo $js_obj;?>));
						//alert(<?php echo $js_obj;?>);
						//json = JSON.parse(json);
						//var str = fil + " - " + idfil + " - " + tov + " - " + idtov;
						//alert(json[fil][idfil][tov][idtov]['nametovar']);
						return false; 
					}
	...
	$(window).load(function() {
		...
		//$("#open-dialog").click(function(){
					...
					//$("#dial").dialog("open");
					//var attr = $this.attr("id");
					//curentid = attr.slice(attr.indexOf("-")+1);
		//		});
...
<div style="max-width:947px;width:947px;; background-color:#203052; color:#fff; position: relative; z-index:99; position: relative; margin-left: -22px; margin-right: -250px;">
<?php
...
//echo $result;
 ...
 foreach ($parametrs["filials"]["19"]['tovary'] as $k => $v) {
	
	$tovarblock = "tovarblock-".$k;
	$tovarpic = "tovarpic-".$k;
	$tovarpic2 = "tovarpic2-".$k;
	$tovarhead="tovarhead".$k;
	$pic_osnast = array();
	$pic_osnast = $v['pic'];
	$tovarprice = "tovarprice-".$k;
	$pechatprice = $v['pechat_price'];
	$pechatpriceottisk = $pechatprice+$v['pechat_pic_otrisovka'];
	$opendialog = "opendialog-".$k;
	$onclickfunc = '';
	//$onclickfunc = "opendialog('filials','19','tovary','".$k."');";
	?>

		<div id=<?php echo $tovarblock ?> style="width:100%; height:250px; margin: 0 0 20px 0; color:#333; background-color:#fff;">
		
		
			<div id=<?php echo $tovarpic ?> style="height:200px;float:left; display:block-inline;">
					<img src=<?php echo $pic_osnast[$v['pref_pic']]['src'] ?> width="200px" height="200px" border="0" />	
			</div>
			<div id=<?php echo $tovarpic2 ?> style="height:200px;float:left; display:block-inline;padding: 50px 0 0 0;position: relative; margin-left: -50px;">
				<img src=<?php echo $v['stamp_pic'] ?> width="100px" height="100px" border="0" />	
			</div>
			
			<div id=<?php echo $tovarhead ?> style="height:200px;float:left; display:block-inline;padding: 10; margin-left: 20px; max-width:350px;">
				<a name="colopr40"><h1 style="color: #0088cc; font-style:italic; font-size:1.5em;">Печать на автоматической оснастке Colop R40</h1></a>
				Материал клише — <?php echo $v['material'] ?><br />
				Оснастка — <?php echo $v['osnastka'] ?><br />
				Производитель — <?php echo $v['maker'] ?>, <?php echo $v['country'] ?><br />
				Штемпельная подушка — <?php echo $v['podushka'] ?><br />
			</div>
		
			<div id=<?php echo $tovarprice ?> style="height:200px;float:left; display:block-inline;padding: 10; margin-left: 20px; max-width:350px; color: #333;">
				<h1 style="color: #0088cc; font-style:italic; font-size:1.5em;">Хочу заказать печать</h1>
				<table cellspacing="0" cellpadding="0">
				<tr style="height:40px;"><td>
				<a style="color:#203052; text-decoration:none;">новая печать:</a></td><td><a style="margin:20px; border:1px solid #203052; background-color:#203052; border-radius: 4px; color:#fff;"> от <?php echo $pechatprice ?> р.</a><br />
				</td></tr>
				<tr><td>
				<a style="color:#203052; text-decoration:none;">печать по оттиску:</a></td><td><a style="margin:20px; border:1px solid #203052; background-color:#203052; border-radius: 4px; color:#fff;"> от <?php echo $pechatpriceottisk ?> р.</a>
				</td></tr>
				</table>
				<input type="button" id=<?php echo $opendialog ?> class=".open-dialog" value="Заказать сейчас" style="width:100%; margin-top:40px; background-color:#203052;border:1px solid #203052; border-radius: 4px; color: #fff; font-size: 2em; height: 2.5em;" <!--onclick=<?php //echo $onclickfunc ?>--> />
			</div>
		</div>
	<div style="height: 200px;">
		...
		<?php echo $v['pref_pic']?> <br /> <?php echo $v['stamp_pic'] ?> <?php echo  $pic_osnast[$v['pref_pic']] ; echo " --- "; 	echo $onclickfunc;?> 
	</div>
